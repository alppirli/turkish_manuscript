...
for (file in files) {
  ...
  # ^ here, aux, part, etc. can stay because we are interested in syntax and not lexical choice
}
...
for (feat in fig_features) {
  ggplot(plot_data_1,
    ...
    geom_line(aes(color = ifelse(feature == feat, classifier, "others")), linewidth = 0.5) +
...
class_labels <- c("cosine_c"="cosine (authorial profiles)", "delta_c"="delta (authorial profiles)",
                  "cosine"="cosine (discrete works)", "delta"="delta (discrete works)")
